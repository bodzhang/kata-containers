...
| `storages` | Raw non-rootfs request storages are diagnostic evidence only and are cleared from standalone compiler output. Rootfs identities become per-container marker storages. A storage-bearing rootfs must be guest-pull, a valid EROFS lower/upper component, or read-only single-layer dm-verity; unsupported or malformed rootfs storage always fails generation. Workload volume classes that require Agent `Storage` entries fail closed in standalone output until trusted workload/profile lowering replaces capture-derived storage templates. |
| `devices` | Raw request devices are diagnostic evidence only. Workload YAML supplies exact guest paths for declared `volumeDevices`. For each declared NVIDIA pGPU resource, the compiler emits an unsuffixed VFIO requirement instead of pinning runtime-assigned device numbers or PCI paths. This is request-shape enforcement, not physical-device identity. |
