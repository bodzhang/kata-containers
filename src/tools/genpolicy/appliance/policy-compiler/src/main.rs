...
fn compile_request_devices(
    _captured: &[CapturedAgentDevice],
    generated: Vec<agent::Device>,
    _vfio_device_path: &str,
) -> Result<Vec<agent::Device>> {
    // Captured IDs, host/VM paths, transport types, PCI addresses, and options
    // are untrusted runtime observations. The policy carries only the guest
    // container paths, counts, and profile device types derived above from the
    // workload YAML and reviewed settings.
    Ok(generated)
}
...
fn normalize_mounts(
    ...
    for captured in &capture.mounts {
        if let Some(template) = templates.get(captured.destination.as_str()) {
            ...
            bail!(
                "captured mount has no YAML or reviewed UVM profile normalization: {}",
...
fn run(args: Args) -> Result<()> {
    ...
    for (basename, tagged_request) in tagged_requests {
        ...
        // Volume storage policy is generated by typed runtime-rs fragments.
        // Captured storage drivers, sources, device addresses, mount points,
        // and options are transport observations and never become authority.
        container.storages.clear();
...
mod tests {
    ...
    #[test]
    fn request_devices_ignore_captured_host_identity() {
        ...
        assert!(devices[0].type_.is_empty());
        assert!(devices[0].vm_path.is_empty());
        assert!(devices[0].options.is_empty());
    ...
    #[test]
    fn request_devices_do_not_authorize_undeclared_vfio_capture() {
        let vfio_path = "/dev/vfio/devices/vfio";
        ...
        let devices = compile_request_devices(&captured, Vec::new(), vfio_path).unwrap();
        assert!(devices.is_empty());
    }

    #[test]
    fn request_devices_use_declared_vfio_count() {
        let vfio_path = "/dev/vfio/devices/vfio";
        let captured = vec![CapturedAgentDevice {
            id: "vfio0".to_string(),
            field_type: "vfio-pci-gk".to_string(),
    ...
    #[test]
    fn compiled_devices_contain_only_uvm_policy_fields() {
        let settings_path =
            Path::new(env!("CARGO_MANIFEST_DIR")).join("../../genpolicy-settings.json");
        let settings = Settings::new(settings_path.to_str().unwrap());
        let capture: CapturedSpec = serde_json::from_value(json!({
        ...
        assert!(compiled.storages.is_empty());
        assert_eq!(compiled.devices.len(), 2);
        assert_eq!(compiled.devices[0].container_path, "/dev/data");
        assert!(compiled.devices[0].id.is_empty());
        assert!(compiled.devices[0].type_.is_empty());
        assert!(compiled.devices[0].vm_path.is_empty());
        assert!(compiled.devices[0].options.is_empty());
        assert_eq!(
            compiled.devices[1].container_path,
            settings.devices.vfio.device_path
        );
        assert!(compiled.devices[1].id.is_empty());
        assert!(compiled.devices[1].vm_path.is_empty());
        assert!(compiled.devices[1].options.is_empty());
    }
...
"#,
    ...
    #[test]
    fn fragment_fixture_matches_compiler_device_intent() {
        let workload = Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../tests/fixtures/device-direct-volume-workload.yaml");
        let settings_path =
            Path::new(env!("CARGO_MANIFEST_DIR")).join("../../genpolicy-settings.json");
        let settings = Settings::new(settings_path.to_str().unwrap());
        ...
        assert_eq!(workload.nvidia_pgpu_count, 2);
    }
