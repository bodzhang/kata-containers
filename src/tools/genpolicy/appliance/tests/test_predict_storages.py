...
class PredictStoragesTests(unittest.TestCase):
    ...
    def test_collect_uses_meta_cid_and_annotation_sid(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(captured["rootfs_mounts"], "")
    ...
    def test_collect_passes_rootfs_mounts_artifact(self):
        with tempfile.TemporaryDirectory() as temporary:
            raw = Path(temporary)
            ...
            rootfs_artifact = raw / "0001-r.rootfs-mounts.json"
            rootfs_artifact.write_text("[]", encoding="utf-8")
