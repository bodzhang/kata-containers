...
bundle to the code and configuration that produced it. The manifest's
`capture.request_authority` states whether final requests came from the
runtime-rs `RecordingAgent` or whether raw OCI is the authoritative runc
boundary and reconstructed Agent requests are diagnostic only.
...
For the guest-pull profile, the shim enables runtime-rs's existing
`force_guest_pull` experiment so the native snapshotter's host bind rootfs is
converted by `ResourceManager` into the same guest-pull virtual volume that the
confidential deployment uses. The EROFS profile preserves containerd's real
block mounts instead. The no-VM appliance does not expose a runtime-rs native
profile because an ordinary bind rootfs requires a shared-filesystem transport.
