...
GenPolicy settings and the appliance policy compiler both produce these
...
!!! success "Evaluator operand contract implemented"
  `genpolicy-oci-compiler` and Legacy GenPolicy emit the versioned evaluator
...
## Appliance Completion Criteria

The appliance migration is complete only when:

- `genpolicy-oci-compiler` and Legacy GenPolicy emit the same required
  evaluator contract;
